...
def _copy_psa_libs(source, destination, args):
    ...
    output_dir = join(destination, "test", "lib", "TOOLCHAIN_" + TC_DICT[args.toolchain])
    ...
    test_combine_output = join(output_dir, "libtest_combine" + output_lib_suffix)
    logging.info(
        "Copying file: %s - to - %s"
        % (test_combine, test_combine_output)
    )
